...
function pprf_new_fn( podid, postid, cpodid, authorid , iframeid, poditemid, parentName ){
	if( jQuery.isNumeric( podid ) && jQuery.isNumeric( cpodid ) && jQuery.isNumeric( authorid ) && jQuery.isNumeric( poditemid ) ) {
		...
		jQuery('#fs-repeater-fields-' + cpodid + '-' + poditemid + '-' + 'loader' ).removeClass('hidden');			
		jQuery.post(
			ajax_script.ajaxurl, 
			data_obj, 
			function( re_arr ){	
				jQuery('#fs-repeater-fields-' + cpodid + '-' + poditemid + '-' + 'loader' ).addClass('hidden');					
				...
				if( re_arr['success'] === true &&  typeof return_arr['id'] !== 'undefined' && jQuery.isNumeric( return_arr['id'] ) ){
					...
					jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list' ).append( iframe_str ); 
					// if entries limit, toggle the add new 
					var itemsLeft_int	= jQuery('#fs-repeater-fields-' + cpodid + '-' + poditemid + ' > .pprf-redorder-list > li').length;
					var limit_int	=	parseInt( jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + '-entry-limit' ).val() );
					if( limit_int != 0 && itemsLeft_int >= limit_int ){
						jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + '-add-new' ).addClass('hidden');							
					}						

				}
				// if add a new one, activeate the live items tab
				jQuery( '#fs-repeater-fields-tabs-' + cpodid + '-' + poditemid + ' .pprf-tab .dashicons-portfolio').click();
...
//console.log($item_arr);
	...
	if( jQuery('#fs-repeater-trash-info-' + cpodid + '-' + poditemid ).data('enable-trash') == 1 ){		
		if( typeof $item_arr['trashed'] != 'undefined' && $item_arr['trashed'] == 1 ){
			...
			if( jQuery('#fs-repeater-fields-tabs-' + cpodid + '-' + poditemid + ' .pprf-tab.active .dashicons').hasClass('dashicons-portfolio') ) {
			...
			if( jQuery('#fs-repeater-fields-tabs-' + cpodid + '-' + poditemid + ' .pprf-tab.active .dashicons').hasClass('dashicons-trash') ) {
				display_str		= 'display:none;';
			}				
		}
	}	
	...
	var delAct_str 		= '';//jQuery('#fs-repeater-fields-' + cpodid + '-' + poditemid + '-' + 'add-new .pprf-trash-btn' ).data('target');
	...
	var fullUrl_str	 	= fs_PODS_REPEATER_PAGE_URL + 'iframe_id=fs-repeater-edit-' + ids_str + '&podid=' + podid + '&tb=' + cpodid + '&postid=' + postid + '&itemid=' + response_str + '&poditemid=' + poditemid;	
	var html_str   =	
	...
	if( repeated_bln == true ){
			html_str   +=	
						'<div class="button pprf-right-col center pprf-dismiss-btn ' + delAct_str + ' ' + btnTrashed_str + '" data-podid="' + podid + '"  data-postid="' + postid + '"  data-tb="' + cpodid + '"  data-itemid="' + response_str + '"  data-userid="' + authorid + '"  data-iframe_id="fs-repeater-edit-' + ids_str + '"  data-poditemid="' + poditemid + '" data-target="' + ids_str + '" >' + 
							'<span class="dashicons dashicons-dismiss pdt6 mgb0 "></span>' +														
						'</div>' +	
						'<div class="pprf-left pd8">Repeated</div>';			
	} else {
		html_str   +=				
				'<div class="button pprf-right-col center pprf-trash-btn ' + delAct_str + ' ' + btnTrashed_str + '" data-podid="' + podid + '"  data-postid="' + postid + '"  data-tb="' + cpodid + '"  data-itemid="' + response_str + '"  data-userid="' + authorid + '"  data-iframe_id="fs-repeater-edit-' + ids_str + '"  data-poditemid="' + poditemid + '" data-target="' + ids_str + '" >' + 
					'<span class="dashicons dashicons-trash pdt6 mgb0 "></span>' +
					'<div id="fs-repeater-trash-' + ids_str + '-loader" class="pprf-left hidden mgl5">' +
						'<img src = "' + fs_PODS_REPEATER_URL + '/images/dots-loading.gif" alt="loading" class="mgl8 loading pprf-left"/>' +
					'</div>' +															
				'</div>' +	
				'<div class="button pprf-right-col center pprf-save-btn" data-podid="' + podid + '"  data-postid="' + postid + '"  data-tb="' + cpodid + '"  data-itemid="' + response_str + '"  data-userid="' + authorid + '"  data-iframe_id="fs-repeater-edit-' + ids_str + '" data-poditemid="' + poditemid + '" data-target="' + ids_str + '" >' +
					'<img src = "' + fs_PODS_REPEATER_URL + 'images/save-icon-tran.png" class="pprf-save-icon  mgt8 mgb2"/>' + 	
					'<div id="fs-repeater-save-' + ids_str + '-loader" class="pprf-left hidden mgl5">' +
						'<img src = "' + fs_PODS_REPEATER_URL + 'images/dots-loading.gif" alt="loading" class="mgl8 pprf-left"/>' +										
					'</div>' +
				'</div>' +													
				'<div class="button pprf-edit pprf-row-load-iframe alignright pprf-right-col center pprf-edit-btn" data-target="' + ids_str + '" data-url="' + fullUrl_str + '">' +
					...
					'<div id="fs-repeater-edit-' + ids_str + '-loader" class="pprf-left hidden mgl5">' +
						'<img src = "' + fs_PODS_REPEATER_URL + '/images/dots-loading.gif" alt="loading" class="mgl9 pprf-left"/>' +
					'</div>	' +
				'</div>';
	}
	html_str   +=								
			'</div>' +										   
			'<div>' + 
				'<iframe id="fs-repeater-edit-' + ids_str + '" frameborder="0" scrolling="no" src="" style="display:none; " class="fs-repeater-iframe w100"></iframe>' + 
				'<div id="fs-repeater-edit-expand-' + ids_str + '" class="w100 pprf-left center pdt3 pdb3 pprf-expand-bar pprf-edit-expand" data-target="' + ids_str + '"  style="display:none;">Content missing? Click here to expand</div>' + 
			'</div>' +
	   	  '</div>' +
		'</li>'
	   ;
...
function pprf_delete_item_fn( podid, postid, cpodid, itemid, authorid , iframeid, poditemid, trashed ){
	...
	if( jQuery.isNumeric( podid ) && jQuery.isNumeric( cpodid ) && jQuery.isNumeric( authorid ) && jQuery.isNumeric( itemid ) && jQuery.isNumeric( poditemid )  ) {
		...
		if( passt_bln == true  ){
			...
			if( trashed == 0 ){
				jQuery( '#fs-repeater-edit-' + cpodid + '-' + itemid + '-' + poditemid + '-loader' ).removeClass('hidden');
			} else {				 
				jQuery( '#fs-repeater-trash-' + cpodid + '-' + itemid + '-' + poditemid + '-loader' ).removeClass('hidden');
			}
				...
				function( response_arr ){	
					if( response_arr['success'] === true  ){
						...
						if( rsp_arr.length != 0 ){
							...
							var exp_str		= 'fs-repeater-edit-expand-' + ids_str;
							var iframe_str 	= 'fs-repeater-edit-' + ids_str;		

							if( trashed == 0 ){
								jQuery( '#fs-repeater-edit-' + cpodid + '-' + itemid + '-' + poditemid + '-loader' ).addClass('hidden');
							} else {
								jQuery( '#fs-repeater-trash-' + cpodid + '-' + itemid + '-' + poditemid + '-loader' ).addClass('hidden');
							}
							//jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' #' + iframeid ).remove( );
							if( trashed == 0 ){
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-row-load-iframe .pprf-edit-span').removeClass('dashicons-update');
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-row-load-iframe .pprf-edit-span').addClass('dashicons-edit')
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"]' ).removeClass('pprf-trashed');
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"]' ).addClass('pprf-not-trashed');
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"]' ).css('display', 'none');	
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-trash-btn').addClass('pprf-btn-not-trashed');	
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-trash-btn').removeClass('pprf-btn-trashed');
								
								if( jQuery.trim( jQuery('#' + iframe_str  ).contents().find("body").html() ) != '' ) {
									jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-save-btn .pprf-save-icon').attr('src', fs_PODS_REPEATER_URL + 'images/save-icon.png');
								}
							}
							if( trashed == 1 ){
								...
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"]' ).removeClass('pprf-not-trashed');
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"]' ).addClass('pprf-trashed');
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"]' ).css('display', 'none');
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-row-load-iframe .pprf-edit-span' ).addClass('dashicons-update');
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-row-load-iframe .pprf-edit-span' ).removeClass('dashicons-edit');
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-trash-btn').addClass('pprf-btn-trashed');	
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-trash-btn').removeClass('pprf-btn-not-trashed');	
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"] .pprf-save-btn .pprf-save-icon').attr('src', fs_PODS_REPEATER_URL + 'images/save-icon-tran.png');
								

							}
							if( trashed == 2 ){	
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' #' + iframeid ).parent().parent().remove( );
								jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + ' .pprf-redorder-list li[data-id="' + itemid + '"]' ).remove( );
								// if entries limit, toggle the add new 
								var itemsLeft_int	= jQuery('#fs-repeater-fields-' + cpodid + '-' + poditemid + ' > .pprf-redorder-list > li').length;
								var limit_int	=	parseInt( jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + '-entry-limit' ).val() );
								if( limit_int != 0 && itemsLeft_int < limit_int ){
									jQuery( '#fs-repeater-fields-' + cpodid + '-' + poditemid + '-add-new' ).removeClass('hidden');	
...
function pprf_load_more_fn( target_str, ele_obj ){
	...
	$( '#fs-repeater-fields-' + target_str + ' .pprf-redorder-list li' ).each( function( idx_int ) {
	...
	var data_obj = {
		...
		amount		: 	jQuery('#fs-repeater-amount-' + target_str ).val(),
		start		: 	jQuery('#fs-repeater-start-from-' + target_str ).val()
	};
		...
		function( response_obj ){
			...
			if( response_obj.success == true ){
				...
				for( var i = 0; i < response_obj.data.length; i ++ ){	
					...
					if( jQuery( '#pprf-load-more-wrap-' + target_str + ' .fs-repeater-to-load' ).val() == 'append_to' ){ // only need to check repeatition if it is on Append To.
				...
				if( jQuery( '#pprf-load-more-wrap-' + target_str + ' .fs-repeater-to-load' ).val() == 'append_to' ){
					jQuery( '#fs-repeater-fields-' + data_obj.saved_tb + '-' + data_obj.pod_item_id + ' .pprf-redorder-list' ).append( html_str ); 
				} else {
					jQuery( '#fs-repeater-fields-' + data_obj.saved_tb + '-' + data_obj.pod_item_id + ' .pprf-redorder-list' ).html( html_str ); 
				}
				...
				if( response_obj.data.length != 0 ){
					jQuery('#fs-repeater-start-from-' + target_str ).val( parseInt( data_obj.start ) + parseInt( response_obj.data.length ) );
				}
...
function pprf_odd_even_color_fn( ids_str ){
	...
	jQuery( '#fs-repeater-fields-' + ids_str + ' .pprf-left-col').removeClass('pprf-purple-bg');
	jQuery( '#fs-repeater-fields-' + ids_str + ' .pprf-left-col').removeClass('pprf-white-bg');
	
	if( jQuery( '#fs-repeater-fields-tabs-' + ids_str ).length == 0 ){
		
		jQuery( '#fs-repeater-fields-' + ids_str + ' .pprf-left-col').each( function( idx_int ) {
			if( idx_int % 2 == 0 ){
	...
	jQuery( '#fs-repeater-fields-' + ids_str + ' .pprf-not-trashed').each( function( idx_int ) {
		if( idx_int % 2 == 0 ){
	...
	jQuery( '#fs-repeater-fields-' + ids_str + ' .pprf-trashed').each( function( idx_int ) {
		if( idx_int % 2 == 0 ){
			jQuery( this ).children('.pprf-row').children().children('.pprf-left-col').addClass('pprf-white-bg');
		} else {
			jQuery( this ).children('.pprf-row').children().children('.pprf-left-col').addClass('pprf-purple-bg');
...
jQuery(document).ready( function($) {
	...
	if( $('.pprf-redorder-list.FSrf_order').length != 0 ){
		$('.pprf-redorder-list.FSrf_order').sortable({
...
/*	if( $('.pprf-row').length != 0 ){
	...
	$(document.body).on('click', '.pprf-row-load-iframe', function(){	
		...
		if( $( this ).children('.pprf-edit-span').hasClass('dashicons-update') ){ 
			...
			if( $( this ).hasClass('pprf-add') ){
				...
				iframe_str 	= 'fs-repeater-add-new-' + ids_str;
				exp_str		= 'fs-repeater-add-new-expand-' + ids_str;
			}	
			...
			if( $('#pprf-row-brief-' + ids_str + ' .dashicons' ).hasClass('dashicons-edit') ){		
				...
				$('#' + iframe_str ).on('load', function(){
					...
					//change icon	
					$('#fs-repeater-save-' + ids_str + '-' + 'loader' ).parent().children('.pprf-save-icon').attr('src', fs_PODS_REPEATER_URL + '/images/save-icon.png');
					$('#fs-repeater-save-' + ids_str + '-' + 'loader' ).parent().addClass('pprf-btn-ready');
					$('#fs-repeater-save-' + ids_str + '-' + 'loader' ).addClass('hidden');
	...
	$(document.body).on('click', '.pprf-save-btn', function(){			
		 if( $( this ).hasClass( 'pprf-btn-ready' ) ){
			...
			$('#fs-repeater-save-' + ids_str + '-loader' ).removeClass('hidden');
	  ...
	  */
	 $('.toplevel_page_fs-pods-repeater-field .pods-field-input').on('click keyup change', function(){
...
//		 }
	...
	$(document.body).on('click', '.pprf-tab .dashicons-trash', function(){			
	 	...
	 	$( '#fs-repeater-fields-' + $( this).parent().data('target') + ' .pprf-trashed').css('display', 'block');
	 	$( '#fs-repeater-fields-' + $( this).parent().data('target') + ' .pprf-not-trashed').css('display', 'none');
	...
	$(document.body).on('click', '.pprf-tab .dashicons-portfolio', function(){			
	 	$( '#fs-repeater-fields-' + $( this).parent().data('target') + ' .pprf-trashed').css('display', 'none');
	 	$( '#fs-repeater-fields-' + $( this).parent().data('target') + ' .pprf-not-trashed').css('display', 'block');
	 	$( this ).parent().parent().children('.active').removeClass('active');
	 	$( this ).parent().addClass('active');	 	
	 	pprf_odd_even_color_fn( $( this).parent().data('target') );
	})	 
