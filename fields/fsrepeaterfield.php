...
$isAdmin_bln	=	false;
if( strpos( $_SERVER['REQUEST_URI'], 'wp-admin') && isset( $_GET['page'] ) && $_GET['page'] == 'fs-pods-repeater-field' ){ // is_admin doesn't work for nested fields
	$isAdmin_bln	=	true;	
} {
...
if( !defined( 'fs_PODS_REPEATER_URL' ) || !is_user_logged_in() || !current_user_can('edit_posts')  ){
    ...
    if( $parent_arr ){
		...
		if( ! $allow_bln ){
			...
			foreach( $parentTb_pod->fields as $ck_str => $cField_arr ){
				if( $cField_arr['id'] == $_GET['poditemid']	&& $cField_arr['type'] == 'fsrepeaterfield' ){
			
					if( isset( $cField_arr['options']['fsrepeaterfield_public_access'] ) && $cField_arr['options']['fsrepeaterfield_public_access'] == 1 ){ // allowed for public access
						$allow_bln = true;
					} else {
						// $cField_arr['options']['fsrepeaterfield_role_access'] has no value. It is saved into the _postmeta
...
$allow_bln = apply_filters( 'pprf_load_fs_repeater_allow', $allow_bln, $_GET );
if( !$allow_bln ){
	echo '<div class="mg10">';
	die( apply_filters( 'pprf_load_fs_repeater_allow_msg', esc_html__('You do not have permission to load this item.', 'fs-pods-repeater-field' ) ) );
	echo '</div>';
}
...
if( isset( $_GET['poditemid'] ) && is_numeric( $_GET['poditemid'] ) ){
	$wid_int  = get_post_meta( absint( $_GET['poditemid'] ), 'fsrepeaterfield_field_width' , true);
...
do_action('FSrf_item_top', $get_arr );

if( isset( $_GET['tb'] ) && is_numeric( $_GET['tb'] ) && array_key_exists( 'pod_' . $_GET['tb'], PodsField_fsrepeaterfield::$actTbs_arr ) ) {
	
	$tb_str  = PodsField_fsrepeaterfield::$actTbs_arr[ 'pod_' . $_GET['tb'] ];
	...
	if( isset( $_GET['itemid'] ) && is_numeric( $_GET['itemid'] ) && isset( $_GET['podid'] ) && is_numeric( $_GET['podid'] ) && array_key_exists( 'pod_' . $_GET['podid'], PodsField_fsrepeaterfield::$actTbs_arr ) ) {
		if( $parentTb_pod ){
			$reassign_bln	= false;
			//get current field 
			foreach( $parentTb_pod->fields as $ck_str => $cField_arr ){
				if( $cField_arr['id'] == $_GET['poditemid']	&& $cField_arr['type'] == 'fsrepeaterfield' ){
					$ctb_str	=	$cField_arr['options']['fsrepeaterfield_table'];		
					if( isset( $cField_arr['options']['fsrepeaterfield_allow_reassign'] ) && $cField_arr['options']['fsrepeaterfield_allow_reassign'] == 1 ){
						$reassign_bln	= true;
					}
					break;
				}
			...
			if( $reassign_bln ){
			    ...
			    if ( 0 < $parentTb_pod->total() ) { 
			    	...
			    	$parents_str	.=	'<label class="pprf-left"><strong class="mgr10 mgt5">' . esc_html__('Assign to parent: ', 'fs-pods-repeater-field' ) . '</strong>';
					...
					$parents_str	.=	'</select>';
					$parents_str	.=	'</label>';
			    	$parents_str	.=	'<label class="pprf-left"><strong class="mgr10 mgt5">' . esc_html__('field: ', 'fs-pods-repeater-field' ) . '</strong>';
					...
					$parents_str	.=	'<button id="pprf-reassign-btn" class="pprf-btn pprf-left mgr10">' . esc_html__('Assign', 'fs-pods-repeater-field' ) . '</button>';

					$parents_str	.=	'<div id="pprf-reassign-loader" class="hidden pprf-left">	
											<img src = "' . esc_url( fs_PODS_REPEATER_URL . 'images/dots-loading.gif' ) . '" alt="loading" class="pdt10"/>
										 </div>	';		
					$parents_str	.=	'</label>';									 	
					$parents_str	.=	'</div>';
				}
	...
	echo esc_html__('Invalid table', 'fs-pods-repeater-field' );
}
echo '</div>';
?>
...
<br/>
<br/>
<div class="click-to-close-arrow aligncenter" title="Click this bar to close" ><?php esc_html_e('Click here to collapse', 'fs-pods-repeater-field' ); ?></div>
...
jQuery(document).ready( function($) {
	...
	if( isset( $_GET['success'] ) && $_GET['success'] == 1 && isset( $_GET['iframe_id'] ) && strpos( $_GET['iframe_id'], 'fs-repeater-add-new' ) === 0 ){
		global $wpdb;
	...
	$('.fs-repeater-field-delete-bn').on('click', function(){
