...
*/
class fs_Pods_Repeater_Field_Ajax {
	...
	public function pprf_load_newly_added_fn(){
		...
		if ( ! wp_verify_nonce( $_POST['security'], 'fs-pods-repeater-field-nonce' ) ) {
		...
		if( isset( $_POST['podid'] ) && is_numeric( $_POST['podid'] ) && isset( $_POST['cpodid'] ) && is_numeric( $_POST['cpodid'] ) && isset( $_POST['postid'] )  && isset( $_POST['authorid'] ) && is_numeric( $_POST['authorid'] ) && isset( $_POST['poditemid'] ) && is_numeric( $_POST['poditemid'] ) ){
			...
			if( array_key_exists( 'pod_' . $_POST['cpodid'], $tables_arr ) ){
				...
				$where_str   	= '   `t`.`FSrf_parent_pod_id`  = %d
							   	  AND `t`.`FSrf_parent_post_id` = %d
							   	  AND `t`.`FSrf_pod_field_id`   = %d '; 				
				//$query_str  	= $wpdb->prepare( 'SELECT id FROM `' . $table_str . '` AS t WHERE `t`.`FSrf_categories` REGEXP "(:\"' . $_POST['podid'] . '.' . $_POST['postid'] . '.' . $_POST['poditemid'] . '\";{1,})" AND `t`.`FSrf_author` = %d ORDER BY `t`.`id` DESC LIMIT 0, 1;' , array( $_POST['authorid'] ) );	
				...
				$query_str  	= $wpdb->prepare( 'SELECT t.`id` ' . $qtitle_str . ' 
													FROM `' . $table_str . '` AS t 
													' . $join_str . '
													WHERE ' . $where_str . ' AND `t`.`FSrf_author` = %d 
		...
		if( isset( $_POST['podid'] ) && is_numeric( $_POST['podid'] ) && isset( $_POST['cpodid'] ) && is_numeric( $_POST['cpodid'] ) && isset( $_POST['postid'] ) && isset( $_POST['authorid'] ) && is_numeric( $_POST['authorid'] ) && isset( $_POST['itemid'] ) && is_numeric( $_POST['itemid'] ) && isset( $_POST['poditemid'] ) && is_numeric( $_POST['poditemid'] ) ){
			...
			if( array_key_exists( 'pod_' . $_POST['cpodid'], $tables_arr ) ){
				...
				$where_str   	= '   `t`.`FSrf_parent_pod_id`  = %d
							   	  AND `t`.`FSrf_parent_post_id` = %d
							   	  AND `t`.`FSrf_pod_field_id`   = %d '; 						
				//$query_str  	= $wpdb->prepare( 'SELECT id FROM `' . $table_str . '` AS t WHERE `t`.`FSrf_categories` REGEXP "(:\"' . $_POST['podid'] . '.' . $_POST['postid'] . '.' . $_POST['poditemid'] . '\";{1,})" AND `t`.`id` = %d ORDER BY `t`.`id` DESC LIMIT 0, 1;' , array( $_POST['itemid'] ) );	
				...
				//echo $query_str;
					...
					if( isset( $_POST['trash'] ) && $_POST['trash'] === '0' )	{
						...
						$query_str  	= $wpdb->prepare( 'UPDATE `' . $table_str . '` SET `FSrf_trash` = 0 WHERE `id` = %d;' , array( $_POST['itemid'] ) );
						$deleted_bln 	=	$wpdb->query( $query_str );
						$del_str		=	'restore';
					}					
					if( isset( $_POST['trash'] ) && $_POST['trash'] === '1' )	{ // if $_POST['trash'] == 1, the table should be already updated
						
						$query_str  	= $wpdb->prepare( 'UPDATE `' . $table_str . '` SET `FSrf_trash` = 1 WHERE `id` = %d;' , array( $_POST['itemid'] ) );
						$deleted_bln 	=	$wpdb->query( $query_str );
						$del_str		=	'trash';
					}
	...
	public function pprf_load_more_fn(){
		...
		// order
		$orderBy_str=	'CAST( `FSrf_order` AS UNSIGNED )';
		if( $_POST['order_by'] !== 'FSrf_order' ){
